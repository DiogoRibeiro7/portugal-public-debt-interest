...
a subscription response whose estimate is a null. Both are therefore
...
each cut date, and predict forward from that state. The rate fed forward is the
...
The estimated kernel does not systematically beat the benchmark. It has the
lower mean absolute error at one of the three cut dates and the higher error at
the other two, and every margin is small relative to the error levels. The one
it wins is \BacktestEstTwentyOne\ against \BacktestWamTwentyOne\ basis points
at the 2021 cut --- the cut that places the entire tightening episode out of
sample, and therefore the one where a behavioural channel has something to do.
That is suggestive and it is a single cut date with four annual observations;
it is not evidence of forecasting skill.

That comparison is only meaningful because the behavioural channel is live in
the backtest. An earlier implementation built the kernel at zero shock, which
silences the channel whatever response is supplied, and applied each year's
yield to the whole cumulative repriced share rather than to the cohort that
repriced. Both are corrected here, and the correction moved the ranking: the
earlier version reported the kernel winning at two cut dates for reasons that
had nothing to do with its economics. The honest reading is that the kernel is
