...
below. Net outflow is retained as its own series, but the regression outcome is
the lower bound $\max(\text{net flow}, 0)$, so in a month of net outflow the
dependent variable is exactly zero. That zero is a valid \emph{bound} on gross
subscription, not an observation that no repricing occurred. The coefficient is
therefore an association in the observable lower-bound process, not a point
estimate of the gross flow response, and it is not interpreted as one below.
...
lower-bound process --- and not as an identified behavioural response. Nothing
downstream in this paper treats it as identified: the kernel's central
behavioural effect is set to zero and the fitted response enters only as a
sensitivity band.
...
The scenario kernel has the lower mean absolute error at all three cut dates,
by \emph{very} small margins: under a basis point at 2018 and 2021 and under a
single basis point at 2014, against error levels an order of magnitude larger.
...
before arriving here --- successively because the kernel was built at zero
