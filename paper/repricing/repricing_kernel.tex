...
pass-through range becomes once those clocks are separated. The paper no longer
treats the behavioural coefficient as a central correction: the competing-return
spread is not estimated precisely, the bootstrap interval for asymmetry is
[\AsymmetryLow, \AsymmetryHigh], and the kernel does not beat the maturity
benchmark out of sample. The raw episode is nevertheless informative, because
...
central behavioural effect is set to zero in the kernel tables because the
spread coefficient is imprecise, the bootstrap interval for asymmetry covers
zero, and the backtest does not establish a forecasting gain over the maturity
benchmark. Behavioural responses are carried as sensitivity bounds instead.
...
A falsification test provides no evidence of one particular contamination
channel: the fixed-rate share, a portfolio statistic no household observes,
loads at \PlaceboCoef\ ($p = \PlaceboP$) and leaves the other coefficients
essentially unchanged. Failing to reject a placebo association does not
establish that the identifying variation is valid, and with one dominant
episode and an imprecise model it carries little power. It is a diagnostic,
not a validation.
...
it is the null of Section \ref{sec:estimation}. We report behavioural upside
...
The estimated kernel does not beat the benchmark. Once the backtest is
vintage-consistent and built from each cut date's own portfolio state, the
constant-hazard benchmark has the lower mean absolute error at \emph{every}
cut date, though the margins remain small relative to the error levels. An
earlier version of this exercise reported the kernel winning at two of three
cuts; that ranking was an artefact of applying each year's yield to the whole
cumulative repriced share. The 2021 cut places the entire
tightening episode out of sample, which is where a behaviourally responsive
kernel would have the best chance of separating itself; it does not. That
comparison is only meaningful because the behavioural channel is live in the
backtest --- an earlier implementation built the kernel at zero shock, which
silences the channel whatever response is supplied, so the same sentence was
previously unsupported by the code that produced the table. The honest reading
is that the kernel is
