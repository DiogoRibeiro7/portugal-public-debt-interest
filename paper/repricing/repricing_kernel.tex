...
refixing-risk windows for Portugal, and Table
\ref{tab:refixing-comparison} compares those published windows with the
scenario kernel. This is a benchmark, not a full calibration. The ESDM chart is
cumulative and coarse, while the model still has to impose reset timing,
contractual shape and shock loading. The comparison is useful precisely because
it asks whether the scenario kernel is in the neighbourhood of the debt
manager's own refixing view before the paper turns to the WAM benchmark.
...
$K^{B}$ is behavioural repricing under shock $\Delta$. The difference relative
to the maturity proxy is then
\begin{equation}
...
are therefore model-based sensitivities. The claim this paper defends is not
that the difference equals a particular number, but that weighted average
maturity and a mean do not determine a unique repricing path: plausible
composition and timing assumptions generate materially different paths, of
different signs relative to the constant-hazard benchmark.
