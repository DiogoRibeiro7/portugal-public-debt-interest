...
{% if latest_primary_balance is not none %}- Primary balance: **{{ latest_primary_balance | round(2) }}% of GDP**.
- Overall balance: **{{ latest_overall_balance | round(2) }}% of GDP**.
{% endif %}

## Interpretation

The nominal euro amount and the GDP ratio must be read together. The euro amount can rise while the burden relative to GDP remains stable or falls when nominal GDP grows or the debt ratio declines.

The implicit interest rate measures the cost of the outstanding portfolio. It should not be interpreted as the same object as the current ten-year sovereign yield, which affects the portfolio gradually as debt is issued or refinanced.

## Static full-pass-through sensitivities

{{ shock_table }}

These sensitivities are long-run arithmetic effects. They are not one-year forecasts because the whole debt stock is not refinanced immediately.

## Methodological note

The main indicator is Eurostat ESA 2010 general-government interest payable (`D41PAY`). Pre-1995 AMECO observations, when included, are labelled as a linked series because earlier portions can draw on ESA 95 and ESA 79 accounting frameworks. Forecast observations are not mixed with historical observations.
"""
)
